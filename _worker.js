...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `Iranet ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<head>
        ...
        <title>Iranet Eco Panel ${panelVersion}</title>
	...
	<body>
		<h1>Iranet Eco Panel <span style="font-size: smaller;">${panelVersion}</span> 💦</h1>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}#Iranet-Eco', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#Iranet-Eco', false)">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#Iranet-Eco', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/sub/${userID}?app=sfa#Iranet-Eco', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=sfa#Iranet-Eco', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}?app=clash#Iranet-Eco', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=clash#Iranet-Eco', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `Iranet Warp Pro ${index + 1} 🇮🇷` : `Iranet Warp ${index + 1} 🇮🇷`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`Iranet Warp ${index + 1} 🇮🇷`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `warp-ir_${index + 1}` : `Iranet WoW ${index + 1} 🌍`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function buildWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = 'Iranet BPB F - WorkerLess ⭐'
...
async function getFragmentConfigs(env, hostName) {
    ...
    bestPing.remarks = 'Iranet BPB F - Best Ping 💥';
    ...
    bestFragment.remarks = 'Iranet BPB F - Best Fragment 😎';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'Iranet BPB - Warp Pro Best Ping 🚀' : 'Iranet BPB - Warp Best Ping 🚀';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `Iranet Eco - Warp Pro ${index + 1} 🇮🇷` : `Iranet Eco - Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `Iranet Eco - WoW Pro ${index/2 + 1} 🌍` : `Iranet Eco - WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? 'Iranet Eco - WoW Pro Best Ping 🚀' : 'Iranet Eco - WoW Best Ping 🚀';
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    config['proxy-groups'][0].proxies = isWarp
        ? ['Iranet Warp Best Ping 🚀', 'Iranet WoW Best Ping 🚀', ...warpOutboundsRemarks, ...wowOutboundRemarks ]
        : ['Iranet Best Ping 💥', ...outboundsRemarks ];

    config['proxy-groups'][1].proxies = isWarp ? warpOutboundsRemarks : outboundsRemarks;
    config['proxy-groups'][1].name = isWarp ? `Iranet Warp Best Ping 🚀`: `Iranet Best Ping 💥`,

    isWarp && config["proxy-groups"].push({
        "name": "Iranet WoW Best Ping 🚀",
        "type": "url-test",
...
async function getSingboxConfig (env, hostName, client, isWarp, isFragment) {
    ...
    if (isWarp) {
        ...
        config.outbounds[0].outbounds = client === 'hiddify'
            ? ["Iranet Warp Pro Best Ping 🚀", "Iranet WoW Pro Best Ping 🚀"]
            : ["Iranet Warp Best Ping 🚀", "Iranet WoW Best Ping 🚀"];
        config.outbounds.splice(2, 0, structuredClone(config.outbounds[1]));
        config.outbounds[1].tag = client === 'hiddify' 
            ? "Iranet Warp Pro Best Ping 🚀"
            : "Iranet Warp Best Ping 🚀";
        config.outbounds[2].tag = client === 'hiddify'
            ? "Iranet WoW Pro Best Ping 🚀"
            : "Iranet WoW Best Ping 🚀";
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('Iranet Chain proxy 🔗')}`;
    ...
    outbounds: [
        {
            type: "selector",
            tag: "proxy",
            outbounds: ["Iranet Best Ping 💥"]
        },
        {
            type: "urltest",
            tag: "Iranet Best Ping 💥",
            outbounds: [],
            url: "https://www.gstatic.com/generate_204",
            interval: "30s",
            tolerance: 50
